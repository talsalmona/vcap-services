...
require "insight_service/insight_error"

VALID_CREDENTIAL_CHARACTERS = ("A".."Z").to_a + ("a".."z").to_a + ("0".."9").to_a

class VCAP::Services::Insight::Node

  include VCAP::Services::Insight::Common
  include VCAP::Services::Insight
  ...
  def initialize(options)
    super(options)
    @insight_port = options[:insight_port] || 5672
    ENV["INSIGHT_NODE_PORT"] = @insight_port.to_s
    @insight_ctl = options[:insight_ctl]
    @insight_server = options[:insight_server]
  ...
  def start
    @logger.info("Starting insight node...")
    start_db
    start_server
  ...
  # Insight has no data to dump for migration
  ...
  def save_instance(instance)
    raise InsightError.new(InsightError::INSIGHT_SAVE_INSTANCE_FAILED, instance.pretty_inspect) unless instance.save
  end

  def destroy_instance(instance)
    raise InsightError.new(InsightError::INSIGHT_DESTORY_INSTANCE_FAILED, instance.pretty_inspect) unless instance.destroy
  end

  def get_instance(instance_id)
    instance = ProvisionedService.get(instance_id)
    raise InsightError.new(InsightError::INSIGHT_FIND_INSTANCE_FAILED, instance_id) if instance.nil?
    instance
  end
  ...
  def cleanup_instance(instance, credentials_list = [])
    ...
    raise InsightError.new(InsightError::INSIGHT_CLEANUP_INSTANCE_FAILED, err_msg.inspect) if err_msg.size > 0
  end

  def start_server
    output = %x[#{@insight_server} -detached]
    # successfuly startup message looks like:
    #   Activating Insight plugins ...
    #   0 plugins activated:
    # possibly with some noise if your setup is a little wonky
    # (lists.insight.com/pipermail/insight-discuss/2011-May/012918.html):
    #   *WARNING* Undefined function gb_trees:map/2
    if !output.index("Activating Insight plugins").nil? && !output.index("plugins activated:").nil?
      sleep 2
      ...
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_START_SERVER_FAILED)
    end
  end

  def stop_server
    output = %x[#{@insight_ctl} stop 2>&1]
    if output.split(/\n/)[-1] == "...done."
      return true
    else
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_STOP_SERVER_FAILED)
    end
  end

  def add_vhost(vhost)
    output = %x[#{@insight_ctl} add_vhost #{vhost} 2>&1]
    if output.split(/\n/)[-1] == "...done."
      return true
    else
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_ADD_VHOST_FAILED, vhost)
    end
  end

  def delete_vhost(vhost)
    output = %x[#{@insight_ctl} delete_vhost #{vhost} 2>&1]
    if output.split(/\n/)[-1] == "...done."
      return true
    else
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_DELETE_VHOST_FAILED, vhost)
    end
  end

  def add_user(username, password)
    output = %x[#{@insight_ctl} add_user #{username} #{password} 2>&1]
    if output.split(/\n/)[-1] == "...done."
      return true
    else
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_ADD_USER_FAILED, username)
    end
  end

  def delete_user(username)
    output = %x[#{@insight_ctl} delete_user #{username} 2>&1]
    if output.split(/\n/)[-1] == "...done."
      return true
    else
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_DELETE_USER_FAILED, username)
    end
  end
  ...
  def get_permissions(vhost, username)
    output = %x[#{@insight_ctl} list_user_permissions -p #{vhost} #{username} 2>&1]
    lines = output.split(/\n/)
    if lines[-1] == "...done."
      if lines.size == 3
        list = lines[1].split(/\t/)
       ...
       raise InsightError.new(InsightError::INSIGHT_GET_PERMISSIONS_FAILED, username)
      end
    else
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_GET_PERMISSIONS_FAILED, username)
  ...
  def set_permissions(vhost, username, permissions)
    output = %x[#{@insight_ctl} set_permissions -p #{vhost} #{username} #{permissions} 2>&1]
    if output.split(/\n/)[-1] == "...done."
      return true
    else
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_SET_PERMISSIONS_FAILED, username, permissions)
    end
  end

  def clear_permissions(vhost, username)
    output = %x[#{@insight_ctl} clear_permissions -p #{vhost} #{username} 2>&1]
    if output.split(/\n/)[-1] == "...done."
      return true
    else
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_CLEAR_PERMISSIONS_FAILED, username)
    end
  end

  def list_users
    output = %x[#{@insight_ctl} list_users 2>&1]
    lines = output.split(/\n/)
    if lines[-1] == "...done."
      users = []
      lines.each do |line|
      ...
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_LIST_USERS_FAILED)
    end
  end

  def list_queues(vhost)
    output = %x[#{@insight_ctl} list_queues -p #{vhost} 2>&1]
    lines = output.split(/\n/)
    if lines[-1] == "...done."
      queues = []
      lines.each do |line|
      ...
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_LIST_USERS_FAILED, vhost)
    end
  end

  def list_exchanges(vhost)
    output = %x[#{@insight_ctl} list_exchanges -p #{vhost} 2>&1]
    lines = output.split(/\n/)
    if lines[-1] == "...done."
      exchanges = []
      lines.each do |line|
      ...
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_LIST_EXCHANGES_FAILED, vhost)
    end
  end

  def list_bindings(vhost)
    output = %x[#{@insight_ctl} list_bindings -p #{vhost} 2>&1]
    lines = output.split(/\n/)
    if lines[-1] == "...done."
      bindings = []
      lines.each do |line|
      ...
      @logger.warn("insightctl error: #{output}")
      raise InsightError.new(InsightError::INSIGHT_LIST_BINDINGS_FAILED, vhost)
    end
  end
