...
require 'insight_service/insight_node'

module VCAP
  module Services
    module Insight
      class Node
         attr_reader :insight_ctl, :insight_server, :local_ip, :available_memory, :max_memory, :mbus
...
describe VCAP::Services::Insight::Node do
  before :all do
    @logger = Logger.new(STDOUT, "daily")
    @logger.level = Logger::ERROR
    @local_db_file = "/tmp/insight_node_" + Time.now.to_i.to_s + ".db"
    @options = {
      :logger => @logger,
      :insight_ctl => "insightmqctl",
      :insight_server => "insightmq-server",
      :insight_port => 5672,
      :ip_route => "127.0.0.1",
      :available_memory => 4096,
      :max_memory => 16,
      :node_id => "insight-node-1",
      :local_db => "sqlite3:" + @local_db_file,
      :mbus => "nats://localhost:4222"
    }
    @default_permissions = "'.*' '.*' '.*'"
    EM.run do
      @node = VCAP::Services::Insight::Node.new(@options)
  ...
  before :each do
    @instance = VCAP::Services::Insight::Node::ProvisionedService.new
    @instance.name = "insight-#{UUIDTools::UUID.random_create.to_s}"
    @instance.plan = :free
  ...
  describe "Node.initialize" do
    it "should set up a insight controller path" do
      @node.insight_ctl.should be @options[:insight_ctl]
    end
    it "should set up a insight server path" do
      @node.insight_server.should be @options[:insight_server]
    end
  ...
  describe "Node.start_server" do
    it "should start insight server correctly" do
      @node.start_server.should be
    end
  end

  describe "Node.insightmqctl.vhost" do
    it "should add vhost successfully using right arguments" do
      @node.add_vhost("test_vhost")
      %x[#{@options[:insight_ctl]} list_permissions -p test_vhost 2> /dev/null].split(/\n/)[-1].should == "...done."
      @node.delete_vhost("test_vhost")
    end

    it "should raise exception when add vhost using wrong arguments" do
      @node.add_vhost("test_vhost")
      expect {@node.add_vhost("test_vhost")}.should raise_error(VCAP::Services::Insight::InsightError)
      @node.delete_vhost("test_vhost")
    end
    ...
    it "should raise exception when delete vhost using wrong arguments" do
      expect {@node.delete_vhost("test_vhost")}.should raise_error(VCAP::Services::Insight::InsightError)
    end
  end

  describe "Node.insightmqctl.user" do
    ...
    it "should raise exception when add user using wrong arguments" do
      @node.add_user("test_user", "test_password")
      expect {@node.add_user("test_user", "test_password")}.should raise_error(VCAP::Services::Insight::InsightError)
      @node.delete_user("test_user")
    end
    ...
    it "should raise exception when delete user using wrong arguments" do
      expect {@node.delete_vhost("test_user")}.should raise_error(VCAP::Services::Insight::InsightError)
    end
  end

  describe "Node.insightmqctl.permissions" do
    before :all do
      @node.add_vhost("test_vhost")
      @node.add_user("test_user", "test_password")
    end
    ...
    it "should raise exception when get permissons using wrong arguments" do
      expect {@node.set_permissions("test_vhost", "no_existed_user", @default_permissions)}.should raise_error(VCAP::Services::Insight::InsightError)
    end
    ...
    it "should raise exception when get permissions using wrong arguments" do
      expect {@node.get_permissions("test_vhost", "no_existed_user")}.should raise_error(VCAP::Services::Insight::InsightError)
    end
    ...
    it "should raise exception when clear permissons using wrong arguments" do
      expect {@node.clear_permissions("test_vhost", "no_existed_user")}.should raise_error(VCAP::Services::Insight::InsightError)
    end
  end

  describe "Node.insightmqctl.stats" do
    before :all do
      @node.add_vhost("test_vhost")
    end
    ...
    it "should raise exception when list users using wrong arguments" do
      node_name = ENV["INSIGHTMQ_NODENAME"]
      ENV["INSIGHTMQ_NODENAME"] = "no_existed_node"
      expect {@node.list_users}.should raise_error(VCAP::Services::Insight::InsightError)
      ENV["INSIGHTMQ_NODENAME"] = node_name
    end
    ...
    it "should raise exception when list queues using wrong arguments" do
      node_name = ENV["INSIGHTMQ_NODENAME"]
      ENV["INSIGHTMQ_NODENAME"] = "no_existed_node"
      expect {@node.list_queues("test_vhost")}.should raise_error(VCAP::Services::Insight::InsightError)
      ENV["INSIGHTMQ_NODENAME"] = node_name
    end
    ...
    it "should raise exception when list exchanges using wrong arguments" do
      node_name = ENV["INSIGHTMQ_NODENAME"]
      ENV["INSIGHTMQ_NODENAME"] = "no_existed_node"
      expect {@node.list_exchanges("test_vhost")}.should raise_error(VCAP::Services::Insight::InsightError)
      ENV["INSIGHTMQ_NODENAME"] = node_name
    end
    ...
    it "should raise exception when list bindings using wrong arguments" do
      node_name = ENV["INSIGHTMQ_NODENAME"]
      ENV["INSIGHTMQ_NODENAME"] = "no_existed_node"
      expect {@node.list_bindings("test_vhost")}.should raise_error(VCAP::Services::Insight::InsightError)
      ENV["INSIGHTMQ_NODENAME"] = node_name
    end
  end
  ...
  describe "Node.unprovision" do
    ...
    it "should raise exception when unprovision an non-existed name" do
      expect {@node.unprovision("non-existed")}.should raise_error(VCAP::Services::Insight::InsightError)
    end
  end
  ...
  describe "Node.bind" do
    ...
    it "should access insightmq server use the returned credential" do
  ...
  describe "Node.unbind" do
    ...
    it "should not access insightmq server after unbinding" do
      expect do
        EM.run do
          AMQP.connect(:host => @binding_credentials["host"],
                     :port => @binding_credentials["port"],
  ...
  describe "Node.save_instance" do
    it "should raise exception when save instance failed" do
      ...
      expect {@node.save_instance(@instance)}.should raise_error(VCAP::Services::Insight::InsightError)
    end
  end
  ...
  describe "Node.destory_instance" do
    it "should raise exception when destroy instance failed" do
      instance = VCAP::Services::Insight::Node::ProvisionedService.new
      expect {@node.destroy_instance(instance)}.should raise_error(VCAP::Services::Insight::InsightError)
    end
  end

  describe "Node.get_instance" do
    it "should raise exception when get instance failed" do
      expect {@node.get_instance("non-existed")}.should raise_error(VCAP::Services::Insight::InsightError)
    end
  end
  ...
  describe "Node.migration" do
    before :all do
      @instance_credentials = @node.provision(:free)
      sleep 1
      @dump_dir = File.join("/tmp/migration/insight", @instance_credentials["name"])
    ...
    it "should not access insightmq server after disable the instance" do
    ...
    it "should access insightmq server in old node after enable the instance" do
  ...
  describe "Node.shutdown" do
    it "should return true when shutdown finished" do
      EM.run do
        ...
        sleep 1
        %x[#{@options[:insight_ctl]} status 2> /dev/null].split(/\n/)[-1].should_not == "...done."
        EM.add_timer(0.1) {EM.stop}
      end
    end
  end
